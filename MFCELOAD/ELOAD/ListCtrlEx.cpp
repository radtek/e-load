// ListCtrlExtand.cpp : implementation file
//
...
CListCtrlExtand::EditorInfo::EditorInfo()
...
CListCtrlExtand::EditorInfo::EditorInfo(PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd *pWnd)
...
CListCtrlExtand::CellInfo::CellInfo(int nColumn)
:m_clrBack(-1)
,m_clrText(-1)
,m_dwUserData(NULL)
,m_nColumn(nColumn)
...
CListCtrlExtand::CellInfo::CellInfo(int nColumn, COLORREF clrBack, COLORREF clrText, DWORD_PTR dwUserData)
:m_clrBack(clrBack)
,m_clrText(clrText)
...
CListCtrlExtand::CellInfo::CellInfo(int nColumn, EditorInfo eiEditor, COLORREF clrBack, COLORREF clrText, DWORD_PTR dwUserData)
:m_clrBack(clrBack)
,m_clrText(clrText)
,m_dwUserData(dwUserData)
,m_eiEditor(eiEditor)
...
CListCtrlExtand::CellInfo::CellInfo(int nColumn, EditorInfo eiEditor, DWORD_PTR dwUserData)
:m_clrBack(-1)
,m_clrText(-1)
,m_dwUserData(dwUserData)
,m_eiEditor(eiEditor)
...
CListCtrlExtand::ColumnInfo::ColumnInfo(int nColumn)
:m_eiEditor()
,m_clrBack(-1)
,m_clrText(-1)
,m_nColumn(nColumn)
...
CListCtrlExtand::ColumnInfo::ColumnInfo(int nColumn, PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd *pWnd)
...
CListCtrlExtand::ItemData::ItemData(DWORD_PTR dwUserData)
:m_clrBack(0xFFFFFFFF)
,m_clrText(0xFFFFFFFF)
,m_dwUserData(dwUserData)
{
}
CListCtrlExtand::ItemData::ItemData(COLORREF clrBack, COLORREF clrText, DWORD_PTR dwUserData)
:m_clrBack(clrBack)
,m_clrText(clrText)
,m_dwUserData(dwUserData)
{
}
CListCtrlExtand::ItemData::~ItemData()
{
...
IMPLEMENT_DYNAMIC(CListCtrlExtand, CListCtrl)

CListCtrlExtand::CListCtrlExtand()
:m_pEditor(NULL)
,m_nEditingRow(-1)
,m_nEditingColumn(-1)
,m_msgHook()
...
BEGIN_MESSAGE_MAP(CListCtrlExtand, CListCtrl)
	ON_NOTIFY_REFLECT_EX(NM_DBLCLK, &CListCtrlExtand::OnNMDblclk)
	ON_NOTIFY_REFLECT_EX(NM_CUSTOMDRAW, &CListCtrlExtand::OnNMCustomdraw)
	ON_NOTIFY_REFLECT(LVN_COLUMNCLICK, &CListCtrlExtand::OnHdnItemclick)
END_MESSAGE_MAP()

// CListCtrlExtand message handlers


// Retrieves the data (lParam) associated with a particular item.
DWORD_PTR CListCtrlExtand::GetItemData(int nItem) const
{
...
// Retrieves the data (lParam) associated with a particular item.
DWORD_PTR CListCtrlExtand::GetItemDataInternal(int nItem) const
{
	return CListCtrl::GetItemData(nItem);
}
int CListCtrlExtand::InsertItem(int nItem, LPCTSTR lpszItem)
{
	int ret = CListCtrl::InsertItem(nItem, lpszItem);
	SetItemData(ret, 0);
	return ret;
}
int CListCtrlExtand::InsertItem(int nItem, LPCTSTR lpszItem, int nImage)
{
	int ret = CListCtrl::InsertItem(nItem, lpszItem, nImage);
	SetItemData(ret, 0);
	return ret;
}
int CListCtrlExtand::InsertItem(const LVITEM* pItem)
{
	int ret = 0;
	LVITEM pI = *pItem;
...
BOOL CListCtrlExtand::SetItemData(int nItem, DWORD_PTR dwData)
{
	ItemData* pData = (ItemData*)GetItemDataInternal(nItem);
	if(!pData)
	{
...
BOOL CListCtrlExtand::DeleteItem(int nItem)
{
	DeleteItemData(nItem);
	return CListCtrl::DeleteItem(nItem);
}
// Removes all items from the control.
BOOL CListCtrlExtand::DeleteAllItems()
{
	int nCount = this->GetItemCount();
	DeleteAllItemsData();
	return CListCtrl::DeleteAllItems();
}
BOOL CListCtrlExtand::DeleteAllItemsData( )
{
...
BOOL CListCtrlExtand::DeleteItemData(int nItem)
{
...
BOOL CListCtrlExtand::DeleteAllColumnInfo( )
{
...
BOOL CListCtrlExtand::DeleteColumnInfo(int nColumn)
{
...
void CListCtrlExtand::SetColumnEditor(int nColumn, PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd* pWnd)
{
...
void CListCtrlExtand::SetColumnEditor(int nColumn, CWnd* pWnd)
{
	SetColumnEditor(nColumn, NULL, NULL, pWnd);
}
void CListCtrlExtand::SetRowEditor(int nRow, CWnd* pWnd)
{
	SetRowEditor(nRow, NULL, NULL, pWnd);
}
void CListCtrlExtand::SetRowEditor(int nRow, PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd* pWnd)
{
...
CListCtrlExtand::ColumnInfo* CListCtrlExtand::GetColumnInfo(int nColumn)
{
	INT_PTR nCount = m_aColumnInfo.GetCount();
...
int CListCtrlExtand::FindItem(LVFINDINFO* pFindInfo, int nStart ) const
{
...
int CListCtrlExtand::GetItemIndexFromData(DWORD_PTR dwData)
{
	LVFINDINFO find;
...
int CALLBACK CListCtrlExtand::CompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	CListCtrlExtand *This = reinterpret_cast<CListCtrlExtand*>(lParamSort);
	ColumnInfo *pColInfo;
	int nSort = 0;
	int nCompare = 0;
	...
	if(This->m_fnCompare && This->m_fnCompare != &CListCtrlExtand::CompareProc) 
	{
...
void CListCtrlExtand::OnHdnItemclick(NMHDR *pNMHDR, LRESULT *pResult)
{
...
BOOL CListCtrlExtand::SortItems(PFNLVCOMPARE pfnCompare, DWORD_PTR dwData)
{
	int nCount = GetItemCount();
...
BOOL CListCtrlExtand::SortOnColumn(int nColumn, BOOL bChangeOrder)
{
	ColumnInfo *pColInfo;
...
BOOL CListCtrlExtand::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
...
BOOL CListCtrlExtand::EnsureSubItemVisible(int nItem, int nSubItem, CRect *pRect)
{
...
BOOL CListCtrlExtand::DisplayEditor(int nItem, int nSubItem)
{
	int nCount = GetItemCount();
	DWORD_PTR dwEditingItemData = 0;
...
CListCtrlExtand::CellInfo* CListCtrlExtand::GetCellInfo(int nItem, int nSubItem)
{
	ItemData* pData = (ItemData*)GetItemDataInternal(nItem);
...
void CListCtrlExtand::HideEditor(BOOL bUpdate)
{
...
int CListCtrlExtand::GetColumnCount(void)
{
...
BOOL CListCtrlExtand::PreTranslateMessage(MSG* pMsg)
{
...
BOOL CListCtrlExtand::OnNMCustomdraw(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CListCtrlExtand::SetRowColors(int nItem, COLORREF clrBk, COLORREF clrText)
{
...
BOOL CListCtrlExtand::AddItem(int ItemIndex, int SubItemIndex, LPCTSTR ItemText, int ImageIndex)
{
	LV_ITEM lvItem;
...
LRESULT CListCtrlExtand::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CListCtrlExtand::SetColumnColors(int nColumn, COLORREF clrBack, COLORREF clrText)
{
...
void CListCtrlExtand::SetCellColors(int nRow, int nColumn, COLORREF clrBack, COLORREF clrText)
{
...
void CListCtrlExtand::PreSubclassWindow()
{
...
BOOL CListCtrlExtand::SetCellData(int nItem, int nSubItem, DWORD_PTR dwData)
{
...
DWORD_PTR CListCtrlExtand::GetCellData(int nItem, int nSubItem)
{
	CellInfo* pCellInfo = GetCellInfo(nItem, nSubItem);
...
void CListCtrlExtand::SetCellEditor(int nRow, int nColumn, PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd* pWnd)
{
	if(nRow < 0 || nRow >= GetItemCount() || nColumn < 0 || nColumn >= GetColumnCount()) return;
	CellInfo* pCellInfo = GetCellInfo(nRow, nColumn);
	if(!pCellInfo)
...
void CListCtrlExtand::SetCellEditor(int nRow, int nColumn, CWnd* pWnd)
{
	SetCellEditor(nRow, nColumn, NULL, NULL, pWnd);
}


void CListCtrlExtand::SetDefaultEditor(PFNEDITORCALLBACK pfnInitEditor, PFNEDITORCALLBACK pfnEndEditor, CWnd* pWnd)
{
...
void CListCtrlExtand::SetDefaultEditor(CWnd* pWnd)
{
	SetDefaultEditor(NULL, NULL, pWnd);
}

void CListCtrlExtand::SetColumnReadOnly(int nColumn, bool bReadOnly)
{
	if(nColumn < 0 || nColumn >= GetColumnCount()) return;
...
void CListCtrlExtand::SetColumnSorting(int nColumn, Sort eSort, Comparer eComparer)
{
...
void CListCtrlExtand::SetColumnSorting(int nColumn, Sort eSort, PFNLVCOMPARE fnCallBack)
{
	if(nColumn < 0 || nColumn >= GetColumnCount() || !(eSort & SortBits)) return;
	ColumnInfo *pInfo = (ColumnInfo*)GetColumnInfo(nColumn);
	if(!pInfo) SetColumnEditor(nColumn, (CWnd*)NULL);
...
void CListCtrlExtand::SetCellReadOnly(int nRow, int nColumn, bool bReadOnly )
{
	if(nRow < 0 || nRow >= GetItemCount() || nColumn < 0 || nColumn >= GetColumnCount()) return;
...
void CListCtrlExtand::SetRowReadOnly(int nRow, bool bReadOnly)
{
...
BOOL CListCtrlExtand::IsColumnReadOnly(int nColumn)
{
	if(nColumn < 0 || nColumn >= GetColumnCount()) return FALSE;
	ColumnInfo *pInfo = (ColumnInfo*)GetColumnInfo(nColumn);
	if( pInfo )
...
BOOL CListCtrlExtand::IsRowReadOnly(int nRow)
{
...
BOOL CListCtrlExtand::IsCellReadOnly(int nRow, int nColumn)
{
...
void CListCtrlExtand::DeleteSelectedItems(void)
{
	int nItem = -1;
...
void CListCtrlExtand::HandleDeleteKey(BOOL bHandle)
{
	m_bHandleDelete = bHandle;
}
void CListCtrlExtand::SelectItem(int nItem, BOOL bSelect)
{
	if(nItem < 0)
	{
		int nIndex = -1;
...
BOOL CListCtrlExtand::DeleteAllColumns(void)
{
	while(DeleteColumn(0));
	return (GetColumnCount() == 0);
}

BOOL CListCtrlExtand::Reset(void)
{
	return (DeleteAllItems() &&
		DeleteAllColumns());
}
int CListCtrlExtand::Compare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	CListCtrlExtand *This = (CListCtrlExtand *)lParamSort;
...
int CListCtrlExtand::CompareInt(LPCSTR pLeftText, LPCSTR pRightText)
{
	return (int)(atol(pLeftText) - atol(pRightText));
}
int CListCtrlExtand::CompareDouble(LPCSTR pLeftText, LPCSTR pRightText)
{
	return (int)(atof(pLeftText) - atof(pRightText));
}
int CListCtrlExtand::CompareString(LPCSTR pLeftText, LPCSTR pRightText)
{
	return CString(pLeftText).Compare(pRightText);
}
int CListCtrlExtand::CompareNumberString(LPCSTR pLeftText, LPCSTR pRightText)
{
...
int CListCtrlExtand::CompareNumberStringNoCase(LPCSTR pLeftText, LPCSTR pRightText)
{
	LONGLONG l1 = atol(pLeftText);
	LONGLONG l2 = atol(pRightText);
	if(l1 && l2 && (l1 - l2))
...
int CListCtrlExtand::CompareStringNoCase(LPCSTR pLeftText, LPCSTR pRightText)
{
	return CString(pLeftText).CompareNoCase(pRightText);
}
int CListCtrlExtand::CompareDate(LPCSTR pLeftText, LPCSTR pRightText)
{
	COleDateTime dL, dR;
...
BOOL CListCtrlExtand::PreCreateWindow(CREATESTRUCT& cs)
{
