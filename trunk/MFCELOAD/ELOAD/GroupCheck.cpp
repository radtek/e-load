...
void CGroupCheck::CheckGroupboxControls()
{
	...
	while (pWnd)
	{
		...
		bool bInnerWnd = false;
		if((rcWnd.left > rcGroupbox.left) && (rcWnd.right < rcGroupbox.right) && 
			(rcWnd.top > rcGroupbox.top) && (rcWnd.bottom < rcGroupbox.bottom)) bInnerWnd = true;

		if ((true == bInnerWnd) && pWnd != this && pWnd != m_pGroupbox)
		{
